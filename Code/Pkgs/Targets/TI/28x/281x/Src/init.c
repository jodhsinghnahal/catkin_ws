...
$Log: init.c $
    ...
    ***********************************************
    ...
    User: TrevorM     Date: 03/02/05  Time: 09:26PM
    Modified to use the SysClk module to set the SYSCLK, LSPCLK, and HSPCLK

    

    ***********************************************
    Revision: NovaPfrmB_TrevorM/4
    User: TrevorM     Date: 02/18/05  Time: 05:25PM
    Added XINTF2 and FLASH memory initializations.  Removed ".init" linker segment,
    and replaced it with "ramfuncs".
    

    ***********************************************
    Revision: NovaPfrmB_TrevorM/3
    User: TrevorM     Date: 02/16/05  Time: 03:47PM
    added more informative comments
    

    ***********************************************
    Revision: NovaPfrmB_TrevorM/2
    User: TrevorM     Date: 02/11/05  Time: 12:14AM
    Enabled External Memory Interface initialization
    

    ***********************************************
    Revision: NovaPfrmB_TrevorM/1
    User: TrevorM     Date: 02/09/05  Time: 03:38PM
    Added compile-time External Memory Wait State configurations
    
    

    ***********************************************
    Revision: NovaPfrmB_JohnB/2
    User: JohnB     Date: 01/24/05  Time: 06:17PM
    Update HAL lib based on code review updates to init
    package.
    ...
    ***********************************************
    ...
    User: HorstM     Date: 01/05/05  Time: 12:49AM
    changed initial pin definitions for PWM 4 & 7 on ezDSP
    
    

    ***********************************************
    Revision: NovaPfrmB_HorstM/1
    User: HorstM     Date: 12/08/04  Time: 08:32PM
    fixed headers/comments;
    moved GetClkSpeed function to this package;
    re-organized functions in a more logical order
    

    ***********************************************
    Revision: NovaPfrmB_HorstM/1
    User: HorstM     Date: 12/07/04  Time: 09:19PM
    added new function(s), moved function definitions around in file 
    (without changing them) for better grouping (match header comments)
...
#include "sysclk.h"         // functions for setting system and periph clocks
#include "xassert.h"
...
#if( defined( CTRL_CONFIG_OPTION_6 ) || defined( CTRL_CONFIG_OPTION7 ) )
...
#define PORTA_PIN3_INIT       PIN_PRIMARY
...
//Port B assignments
#define PORTB_PIN0_INIT       PIN_PRIMARY
...
// Enable Interrupts
#define INIT_INT_EN         ( TRUE )

// External Memory initialization enable
#define INIT_EXTMEM_EN      ( TRUE )

// Internal Flash Memory initialization enable
#define INIT_FLASH_EN       ( TRUE )
...
// Internal Flash Memory Initialization
#define INIT_FLASH_EN       ( TRUE )
...
//*****************************************************************************
// Default External Memory Interface settings

// all memory speed definitions below should be calculated assuming a maximum
// clock speed of 150MHz, since this will be worst-case
#if( CTRL_EXTMEM_SPEED == MEM_SPEED_10_NS )
    #define XTIMING2_XWRLEAD        F28X_XTIMINGX_WAIT_1_STATE 
    #define XTIMING2_XWRTRAIL       F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING2_XWRACTIVE      F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING2_XRDLEAD        F28X_XTIMINGX_WAIT_1_STATE 
    #define XTIMING2_XRDTRAIL       F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING2_XRDACTIVE      F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING2_X2TIMING       F28X_XTIMINGX_X2TIMING_DISABLED
    #define XTIMING2_XSIZE          F28X_XTIMINGX_XSIZE_16BIT
    #define XTIMING2_USEREADY       F28X_XTIMINGX_USEREADY_DISABLED
    #define XTIMING2_READYMODE      F28X_XTIMINGX_XREADY_ASYNCHRONOUS

    #define XTIMING6_XWRLEAD        F28X_XTIMINGX_WAIT_1_STATE 
    #define XTIMING6_XWRTRAIL       F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING6_XWRACTIVE      F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING6_XRDLEAD        F28X_XTIMINGX_WAIT_1_STATE 
    #define XTIMING6_XRDTRAIL       F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING6_XRDACTIVE      F28X_XTIMINGX_WAIT_1_STATE
    #define XTIMING6_X2TIMING       F28X_XTIMINGX_X2TIMING_DISABLED
    #define XTIMING6_XSIZE          F28X_XTIMINGX_XSIZE_16BIT
    #define XTIMING6_USEREADY       F28X_XTIMINGX_USEREADY_DISABLED
    #define XTIMING6_READYMODE      F28X_XTIMINGX_XREADY_ASYNCHRONOUS
    
    #define XINTFCNF2_CLKMODE       F28X_XINTFCNF2_CLKMODE_XTIMCLK_DIV_1	
    #define XINTFCNF2_XTIMCLK       F28X_XINTFCNF2_XTIMCLK_SYSCLKOUT_DIV_1
    #define XINTFCNF2_CLKOFF        F28X_XINTFCNF2_CLKOFF_XCLKOUT_DISABLE  
    #define XINTFCNF2_MPNMC         F28X_XINTFCNF2_MPNMC_MICROCOMPUTER_MODE
    #define XINTFCNF2_HOLD          F28X_XINTFCNF2_HOLD_AUTO_GRANT
    #define XINTFCNF2_WRITEBUFFER_DEPTH     F28X_XINTFCNF2_WRITEBUFFER_3    
#else
    // default XTIMING2 settings
    #define XTIMING2_XRDLEAD        F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_XRDACTIVE      F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_XRDTRAIL       F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_XWRLEAD        F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_XWRACTIVE      F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_XWRTRAIL       F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING2_X2TIMING       F28X_XTIMINGX_X2TIMING_ENABLED
    #define XTIMING2_XSIZE          F28X_XTIMINGX_XSIZE_16BIT
    #define XTIMING2_USEREADY       F28X_XTIMINGX_USEREADY_DISABLED
    #define XTIMING2_READYMODE      F28X_XTIMINGX_XREADY_ASYNCHRONOUS

    // default XTIMING6 settings
    #define XTIMING6_XRDLEAD        F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_XRDACTIVE      F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_XRDTRAIL       F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_XWRLEAD        F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_XWRACTIVE      F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_XWRTRAIL       F28X_XTIMINGX_WAIT_3_STATES
    #define XTIMING6_X2TIMING       F28X_XTIMINGX_X2TIMING_ENABLED
    #define XTIMING6_XSIZE          F28X_XTIMINGX_XSIZE_16BIT
    #define XTIMING6_USEREADY       F28X_XTIMINGX_USEREADY_DISABLED
    #define XTIMING6_READYMODE      F28X_XTIMINGX_XREADY_ASYNCHRONOUS

    #define XINTFCNF2_CLKMODE       F28X_XINTFCNF2_CLKMODE_XTIMCLK_DIV_2
    #define XINTFCNF2_CLKOFF        F28X_XINTFCNF2_CLKOFF_XCLKOUT_DISABLE
    #define XINTFCNF2_MPNMC         F28X_XINTFCNF2_MPNMC_MICROCOMPUTER_MODE
    #define XINTFCNF2_HOLD          F28X_XINTFCNF2_HOLD_AUTO_GRANT
    #define XINTFCNF2_XTIMCLK       F28X_XINTFCNF2_XTIMCLK_SYSCLKOUT_DIV_2
    #define XINTFCNF2_WRITEBUFFER_DEPTH     F28X_XINTFCNF2_WRITEBUFFER_3
#endif

#define XINTF_MAX_WAIT_WRITE_BUFFER 100

//End of Default External Memory Interface settings
//***************************************************************************** 
    
// Peripheral clock settings
//***************************************************************************** 

// For values other than these defaults, set values for CLTL_LSPCLK_HZ and 
// CTRL_HSPCLK_HZ in config.h

// Definition for the Low Speed Peripheral Clock
#ifndef CTRL_LSPCLK_HZ
#define CTRL_LSPCLK_HZ ( CTRL_CPU_MAXSPEED / 2L )
#endif

// Definition for the High Speed Peripheral Clock
#ifndef CTRL_HSPCLK_HZ
#define CTRL_HSPCLK_HZ CTRL_CPU_MAXSPEED
#endif

//End of peripheral clock settings
//***************************************************************************** 
...
void init_fnInitExtMemory(void);
void init_fnInitXTIMING6(void);
void init_fnInitXTIMING2(void);
void init_fnCopyRamFuncs(void);
void init_fnInitFlashMemory(void);
...
Version: 1.10   Date:   Feb 09, 2005    By: Trevor Monk
    - Added External memory interface initializations

Version: 1.20   Date:   Feb 17, 2005    By: Trevor Monk
    - Added Internal Flash memory initialization
...
void INIT_fnSysInit( void )
{
    ...
    // copy the internal RAM functions (ramfuncs) into secure RAM before
    // they are executed.
	init_fnCopyRamFuncs();

	// Configure the internal Flash Memory interface
#if ( INIT_FLASH_EN == TRUE )
	init_fnInitFlashMemory();
#endif

    // Configure the External Memory interfaces
#if ( INIT_EXTMEM_EN == TRUE )
    init_fnInitExtMemory();    
#endif
...
FUNCTION NAME: init_fnCopyRamFuncs
  
PURPOSE: Copies the secure RAM function code from its linker load location
         into its linker run location. 
...
NOTES:
    This function MUST be called prior to calling init_fnInitFlashMemory
    in order to copy the init_fnInitFlashMemory function from its load
    location to its run location in memory.

    The ramfuncs variables must be defined in the linker command
    file.  For example, place the following into the user defined section:
    
      ramfuncs : LOAD = RAMH0_CODE, PAGE = 0
...
Version: 1.00  Date: FEB-17-2005  By: Trevor Monk
    - Initial Release
...
void init_fnCopyRamFuncs(void)
{
    // these variables are externally defined by the linker
    extern Uint32 ramfuncs_loadstart;
    extern Uint32 ramfuncs_loadend;
    extern Uint32 ramfuncs_runstart;
    
    // get pointers to the linker defined addresses
    Uint16 *puiSrc = (Uint16 *)&ramfuncs_loadstart;
    Uint16 *puiDst = (Uint16 *)&ramfuncs_runstart;
    Uint16 len = (Uint32)&ramfuncs_loadend - 
                 (Uint32)&ramfuncs_loadstart;
    
    // Copy the secureRamFuncs section from its load location to its 
    // run location
    while(len--)
        *puiDst++ = *puiSrc++;
}
...
#if ( INIT_EXTMEM_EN == TRUE )
#pragma CODE_SECTION(init_fnInitExtMemory, "ramfuncs")
#pragma CODE_SECTION(init_fnInitXTIMING6, "ramfuncs")

/******************************************************************************

FUNCTION NAME: init_fnInitExtMemory
  
PURPOSE: Initializes the External Memory interfaces 
...
NOTES:
    This function MUST not reside in any External Memory device, since 
    memory device timings can not be changed by code running out of 
    external memory

    The function init_fnCopyRamFuncs MUST be called prior to calling
    this function to ensure that this function is running from secured RAM.

VERSION HISTORY:

Version: 1.00  Date: FEB-07-2005  By: Trevor Monk
    - Initial Release

Version: 1.10  Date: FEB-17-2005  By: Trevor Monk
    - Added XTIMING2 initializations

******************************************************************************/
void init_fnInitExtMemory(void)
{
	// uiCount variable is placed in a register to ensure that it is not 
	// placed into external memory.  We could also use a DATA_SECTION
	// #pragma to place uiCount into internal memory but "register" is easier
    register Uint16 uiCount = 0;  // timeout counter for Write Buffer Empty
	
    // set up the External Memory interfaces 
	
	// ensure that there are no buffered writes to the XINTF
    while( XintfRegs.XINTCNF2.bit.WLEVEL != F28X_XINTFCNF2_WRITEBUFFER_EMPTY )
    {
        if( uiCount++ >= XINTF_MAX_WAIT_WRITE_BUFFER )
            break;
    }

    // Specifies the number of XINTF writes to buffer
    // The CPU is stalled if more writes are pending
    // The CPU will be stalled until the first write begins its cycle on the 
    // XINTF.
	XintfRegs.XINTCNF2.bit.WRBUFF = XINTFCNF2_WRITEBUFFER_DEPTH;		
	
    // Set the output clock frequency	
	XintfRegs.XINTCNF2.bit.CLKMODE = XINTFCNF2_CLKMODE;		
	
    // control output of XCLKOUT on external pin
	XintfRegs.XINTCNF2.bit.CLKOFF = XINTFCNF2_CLKOFF;		
	
    // Enable Microcomputer or Microprocessor state
    // In Microcomputer state (XINTF_MPNMC == 0), (XINTF ZONE 7 disabled, Boot ROM enabled).
	XintfRegs.XINTCNF2.bit.MPNMC = XINTFCNF2_MPNMC;
	
    // Specify how the DSP handles requests from an external device that is driving 
    // both the XHOLD input signal and the XHOLDA output signal low.
	XintfRegs.XINTCNF2.bit.HOLD = XINTFCNF2_HOLD;
	
    // Set the XTIMCLK prescalar value
	XintfRegs.XINTCNF2.bit.XTIMCLK = XINTFCNF2_XTIMCLK;
	
    // initialize bank specific memory settings
    init_fnInitXTIMING6();
	init_fnInitXTIMING2();
    
    // Wait 8 cycles to let the write instructions propagate through
    // the CPU pipeline.  This must be done before the return-from-
...
FUNCTION NAME: init_fnInitXTIMING6
  
PURPOSE: Initializes the EXTINTF6 External Memory interface 
...
{
    //
    // XTIMING6 Device timings
    //
	
    // Write cycle trail period
    XintfRegs.XTIMING6.bit.XWRTRAIL = XTIMING6_XWRTRAIL;	
	
    // Write cycle active wait state period
    XintfRegs.XTIMING6.bit.XWRACTIVE = XTIMING6_XWRACTIVE;	
	
    // Write cycle lead period
    XintfRegs.XTIMING6.bit.XWRLEAD = XTIMING6_XWRLEAD;
	
    // Read cycle trail period
    XintfRegs.XTIMING6.bit.XRDTRAIL = XTIMING6_XRDTRAIL;
	
    // Read cycle active wait state period
    XintfRegs.XTIMING6.bit.XRDACTIVE = XTIMING6_XRDACTIVE;
	
    // Read cycle lead period
    XintfRegs.XTIMING6.bit.XRDLEAD = XTIMING6_XRDLEAD;
	
    // The XREADY signal is ignored when accesses are made to the zone
    XintfRegs.XTIMING6.bit.USEREADY = XTIMING6_USEREADY;
	
    // XREADY input is synchronous for the zone
    XintfRegs.XTIMING6.bit.READYMODE = XTIMING6_READYMODE;
	
    // 16 bit interface (any other value is currently unsupported)
    XintfRegs.XTIMING6.bit.XSIZE = XTIMING6_XSIZE;
	
    // Device timing numbers are scaled 1:1 (i.e. not scaled)
    XintfRegs.XTIMING6.bit.X2TIMING = XTIMING6_X2TIMING;
}

/******************************************************************************

FUNCTION NAME: init_fnInitXTIMING2
  
PURPOSE: Initializes the EXTINTF2 External Memory interface 
...
{
    //
    // XTIMING2 Device timings
    //
	
    // Write cycle trail period
    XintfRegs.XTIMING2.bit.XWRTRAIL = XTIMING2_XWRTRAIL;	
	
    // Write cycle active wait state period
    XintfRegs.XTIMING2.bit.XWRACTIVE = XTIMING2_XWRACTIVE;	
	
    // Write cycle lead period
    XintfRegs.XTIMING2.bit.XWRLEAD = XTIMING2_XWRLEAD;
	
    // Read cycle trail period
    XintfRegs.XTIMING2.bit.XRDTRAIL = XTIMING2_XRDTRAIL;
	
    // Read cycle active wait state period
    XintfRegs.XTIMING2.bit.XRDACTIVE = XTIMING2_XRDACTIVE;
	
    // Read cycle lead period
    XintfRegs.XTIMING2.bit.XRDLEAD = XTIMING2_XRDLEAD;
	
    // The XREADY signal is ignored when accesses are made to the zone
    XintfRegs.XTIMING2.bit.USEREADY = XTIMING2_USEREADY;
	
    // XREADY input is synchronous for the zone
    XintfRegs.XTIMING2.bit.READYMODE = XTIMING2_READYMODE;
	
    // 16 bit interface (any other value is currently unsupported)
    XintfRegs.XTIMING2.bit.XSIZE = XTIMING2_XSIZE;
	
    // Device timing numbers are scaled 1:1 (i.e. not scaled)
    XintfRegs.XTIMING2.bit.X2TIMING = XTIMING2_X2TIMING;
}

#endif//End #if( INIT_EXTMEM_EN == TRUE )
#endif //#ifndef CTRL_LOADER_INIT

#ifndef CTRL_LOADER_INIT  //Loader specific conditional compile

#if ( INIT_FLASH_EN == TRUE )
#pragma CODE_SECTION(init_fnInitFlashMemory, "ramfuncs")

/******************************************************************************

FUNCTION NAME: init_fnInitFlashMemory
  
PURPOSE: Initializes the Internal Flash Memory Interface 
...
NOTES:
    ...
    The flash register initialization code MUST run from secured RAM 
    (e.g. L0 or L1 SARAM) or the initialization code will be unable to 
    access the flash registers.  Note that the CSM is always secured at device
    reset, even if you are using dummy passwords of 0xFFFF.
    
    Refer to TI Application Note SPRA958D "Running an Application from
...
void init_fnInitFlashMemory(void)
{
    ...
    // Flash set to active mode
    FlashRegs.FPWR.bit.PWR = F28X_FPWR_PUMP_BANK_ACTIVE;
    
    // Clear the 3VSTAT bit
    FlashRegs.FSTATUS.bit.V3STAT = F28X_FSTATUS_CLEAR_V3STAT;
    
    // Sleep to standby cycles
    FlashRegs.FSTDBYWAIT.bit.STDBYWAIT = F28X_FSTDBYWAIT_MAX_WAIT;
    
    // Standby to active cycles
    FlashRegs.FACTIVEWAIT.bit.ACTIVEWAIT = F28X_FSTDBYWAIT_MAX_WAIT;
    
    // Random access waitstates
    FlashRegs.FBANKWAIT.bit.RANDWAIT =  F28X_FBANKWAIT_5_STATES;
    
    // Paged access waitstates
    FlashRegs.FBANKWAIT.bit.PAGEWAIT =  F28X_FBANKWAIT_5_STATES;
    
    // Random access waitstates
    FlashRegs.FOTPWAIT.bit.OTPWAIT = F28X_FBANKWAIT_5_STATES;
    
    // Enable the flash pipeline
    FlashRegs.FOPT.bit.ENPIPE = F28X_FOPT_PIPELINE_ENABLED;
    
    // disallow protected register access
    DEVICE_mACCESS_DIS();
    
    // Force a complete pipeline flush to ensure that the write to the
    // last register configured occurs before returning.  The safest thing
    // to do is to wait 8 full cycles.
	asm ("      NOP");
	asm ("      NOP");
	asm ("      NOP");
	asm ("      NOP");
...
#endif//End #if( INIT_FLASH_EN == TRUE )
#endif //#ifndef CTRL_LOADER_INIT
...
Version: 1.01  Date: June 18, 2004 By: Brent Tokarchuk
    ...
    -   Changed the default state of the peripheral clocks to OFF for power
        saving.
Version: 1.10  Date: Mar 02, 2005  By: Trevor Monk
    -   changed configuration of LSPCLK, and HSPCLK to use the SysClk module.
...
void init_fnInitSystemControlRegs( void )
{
    ...
    SYSCLK_fnSetSysClk( CTRL_CPU_MAXSPEED );
    
    // Configure the High-speed peripheral clock (HSPCLK)
    SYSCLK_fnSetHiSpeedPeriphClk( CTRL_HSPCLK_HZ );
    
    // Configure the Low-speed peripheral clock (LSPCLK)
    SYSCLK_fnSetLoSpeedPeriphClk( CTRL_LSPCLK_HZ );

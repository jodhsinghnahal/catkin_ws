...
#include "includes.h"
#endif
#include "DSP281x_Device.h" // TI Device definitions
...
void drvapi_fnTransmitCB( uchar8 Channel )
{
    ...
    if ( DRVAPI_bOK_to_send( Channel ) )
    {
        ...
        {
            status = CANBUF_fnTxExtract( XB_VIP_BUF, &zTxBuf);
            
            if ( status == CANBUF_BUFFER_EMPTY )
            {
                status = CANBUF_fnTxExtract( XB_REG_BUF, &zTxBuf);
            }

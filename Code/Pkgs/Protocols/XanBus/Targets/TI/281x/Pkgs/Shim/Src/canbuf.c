...
$Log: canbuf.c $
    ...
    ***********************************************
    ...
    User: BaldeeshK     Date: 08/30/05  Time: 05:43PM
    - Changed operation to use heap memory for allocation instead of statically
    defining it
...
tucBOOL CANBUF_fnCtor( uchar8 ucPort )
{
    ...
    assert( ucPort < TI_281x_MAX_PORTS );
    
    // locate memory for Rx buffer on the heap
  	ptzRxBuf = XBMEM_fnMalloc( CANBUF_TI_281x_RX_PORT_BUFS * 
    ...
    if ( ptzRxBuf != NULL )
    {
       ...
       // Initialize the receive fifo to empty and at the begining
        ...
        ptzRxFifo->ucMax = CANBUF_TI_281x_RX_PORT_BUFS;
        ptzRxFifo->ucCount = 0;
        ptzRxFifo->ucPut = 0;
        ptzRxFifo->ucGet = 0;
    ...
    ptzTxBuf = XBMEM_fnMalloc( CANBUF_TI_281x_TX_PORT_BUFS * 
    ...
    {  
        ...
        ptzTxFifo->ucMax = CANBUF_TI_281x_TX_PORT_BUFS;
        ptzTxFifo->ucCount = 0;
        ptzTxFifo->ucPut = 0;
        ptzTxFifo->ucGet = 0;
    }  
    ...
    {  
        // Get local pointer to receive fifo for convenience
        ptzTxFifo = &CANBUF_atzTxFifoList[ XB_VIP_BUF ];
        ...
        ptzTxFifo->ucPut = 0;
        ptzTxFifo->ucGet = 0;
        XBMEM_fnSetPriorityDobStatus( XBMEM_VIP_BUF_ENABLE );
    }
    else
    {
        //Failure

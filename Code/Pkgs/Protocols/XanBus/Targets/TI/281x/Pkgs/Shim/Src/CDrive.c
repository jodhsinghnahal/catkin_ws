...
$Log: CDrive.c $
    ...
    ***********************************************
    Revision: XanBus_TrevorM/1
    User: TrevorM     Date: 04/20/07  Time: 01:40PM
    Added support to the Xanbus library for the TI architecture, for a receive
    data callback to the application layer on reception of a CANFRAME.  This
    functionality was provided to allow an application to forward CAN packets
    over TCP/IP.
...
void (*pRxCallback)( uint32 data ) = NULL;

//**********************************************************************
// TFXDRV_i16InitRxCallback
//**********************************************************************
TFXCAN_RETURNS TFXDRV_i16InitRxCallback(void (*pFn)(uint32 data))
{
    pRxCallback = pFn;
...
TFXCAN_RETURNS TFXDRV_i16SendFrame (CANPORT u8PortNumber, CANFRAME *pFrame, INT16 *pi16MessageID )
{
    ...
    CANPORT u8HardwarePort;

    if ((u8PortNumber == 0) || (u8PortNumber == 1))
    {
        u8HardwarePort = 0;    
    }

    if ((u8PortNumber == 2) || (u8PortNumber == 3))
    {
        u8HardwarePort = 1;    
    }
    ...
    if ( ret == TFXCR_OK)
    {
        ...
        DRVAPI_vDisable_int(u8HardwarePort); // Disable interrupts
        iSts = CANBUF_fnTxInsert( u8PortNumber, &zTxBuf );
        DRVAPI_vEnable_int(u8HardwarePort);  // enable interrupts
        if( iSts == CANBUF_BUFFER_FULL )
        {
            // The circular buffer is full.  We have to drop this message.
            DRVAPI_fnCountTxLost( u8HardwarePort );
...
TFXCAN_RETURNS TFXDRV_WriteFrame(CANPORT u8PortNumber)
{
    ...
    DRVAPI_vDisable_int(u8HardwarePort); // Disable interrupts
    if(DRVAPI_bOK_to_send(u8HardwarePort))
    {
        iSts = CANBUF_fnTxExtract( u8PortNumber,
                                   &zTxBuf );
        if( iSts != CANBUF_BUFFER_EMPTY )
        {
            if( DRVAPI_iXmit_object( u8HardwarePort, &zTxBuf ) != 0 )
            {
...
static BOOL bIsPortValid(CANPORT u8PortNumber)
{
    return u8PortNumber < TI_281x_MAX_PORTS ? TRUE : FALSE;
}

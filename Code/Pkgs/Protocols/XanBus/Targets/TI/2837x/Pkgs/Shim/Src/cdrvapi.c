...
$Log: cdrvapi.c $
    ...
    ***********************************************
    ...
    User: HollyZ     Date: 02/09/09  Time: 10:54PM
    Removed the time filtering feature for updating the CAN Tx status in the
    status callback function; Get the CAN transmitter block status from the
    CAN driver API

    ***********************************************
    Revision: XanBus_BaldeeshK/1
    User: BaldeeshK     Date: 07/29/05  Time: 04:36PM
    - Added call to status callback function to make sure we are continually
    updating our status
...
#include "includes.h"
#endif
#include "F2837xS_device.h" // TI Device definitions
#include "DeviceMacro.h"    // TI Device Control Macros
...
// Flag that ISR is active
static volatile tucBOOL drvapi_ucISRActive = FALSE;
...
Version: 1.03  Date: 02/09/09  By: HollyZ
    - Added the init for the TxTimeOut Arrary
*******************************************************************************/
...
void DRVAPI_vEnable_int( CANPORT Channel )
{
    ...
    // Don't enable interrupts if we're in an ISR
    if( !drvapi_ucISRActive )
    {
        DEVICE_mINT9_EN();
    }
}
...
Version: 1.01  Date: 07/31/03  By: dalem
    - Created
Version: 1.02  Date: 02/09/09  By: HollyZ
    - using the tx status register value to indicate the bus state
*******************************************************************************/
...
uchar8 DRVAPI_fnBusState( CANPORT Channel )
{
    ...
    // return the current Tx Status from the TxStatus register
    return( CANDRV_fnTxStatus( Channel,
                               NULL ) );
}
...
Verson:  1.03  Date: 07/25/05  By: Baldeesh Khaira
    - Added call to update status every tick
Version: 1.04  Date: 02/09/09  By: HollyZ
    - Updated as a result of removing the status filting in drvapi_fnStatusCB()
...
Version: 1.01  Date: 02/09/09  By: HollyZ
    - Get the Tx transmitter block status from the CANDRV api
...
void drvapi_fnReceiveCB( uchar8 Channel )
{
    ...
    // Flag an active interrupt
    drvapi_ucISRActive = TRUE;
    ...
    // Interrupt no longer active
    drvapi_ucISRActive = FALSE;
}
...
void drvapi_fnAbortCB( uchar8 Channel )
{
    ...
    // Don't bother trying to send another frame
...
void drvapi_fnStatusCB( uchar8 Channel )
{
    ...
    // Read tx status
    teStatus = CANDRV_fnTxStatus( Channel, NULL );
   ...
   // If tx status has changed
    ...
    // Read rx status
    teStatus = CANDRV_fnRxStatus( Channel, NULL );

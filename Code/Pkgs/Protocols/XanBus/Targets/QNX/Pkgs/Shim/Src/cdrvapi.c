                                ...
                                uchar8  ucBitRate )
{
    ...
    syslog( LOG_INFO, "DRVAPI_i16InitController");
...
uchar8 DRVAPI_fnBusState( CANPORT Channel )
{
    ...
    CANDRV_teSTATUS txStatus = atzCanCommStats[ Channel ].ucLastTxStatus;

    /* Based on teleflex layer(CDrive.c) and XBMSG_fnGetBusState function,
    CANDRV_eSTATUS_WARNING(Error active) should be categrized as
    CANDRV_eSTATUS_OK. */
    if( atzCanCommStats[ Channel ].ucLastTxStatus == CANDRV_eSTATUS_WARNING )
    {
        txStatus = CANDRV_eSTATUS_OK;
    }
...
Version: 1.04  Date: 14/02/17  By: Jon Liu
    - Added CAN device status update for QNX.
...
void DRVAPI_fnStatusTick( CANPORT Channel )
{
    ...
    CANDRV_tzDEV_STATS CANDevSts;
    drvapi_fnTxTimeOut( Channel );

    if ( Channel < MAX_PORTS )
    {
        ret = CANDRV_fnGetDevStatus( Channel, &CANDevSts );
        if( ret ==  CANDRV_eRET_SUCCESS )
        {
            memcpy( &atzCanCommStats[ Channel ],
                    &CANDevSts,
                    sizeof( CAN_tzCOMM_STATS) );
        }
    }

...
/*! Structure of CAN device status */
typedef struct zCAN_DEV_STATS
{
    ...
    uint16 uiTxTimoutCnt;       // Count of tx transmission timeout
} CANDRV_tzDEV_STATS;
...
//
// Purpose:
//    Determine if the CAN driver is transmitting a frame
// Inputs:
//    ucChannel - which CAN channel
// Outputs:
//    TRUE - CAN driver is transmitting a frame
//    FALSE - CAN driver is not transmitting a frame
...
//
// Purpose:
//    Determine if the CAN BUS is ON or OFF
// Inputs:
//    ucChannel - which CAN channel
// Outputs:
//    TRUE - CAN driver is in BUS ON state
//    FALSE - CAN driver is in BUS OFF state
...
//
// Purpose:
//     Update the CAN device status variable and copy it to ptrCANDevSts
// Inputs:
//     ucChannel - which CAN channel
// Outputs:
//     ptrCANDevSts:
//         The CAN device status variable to copy to.
//     CANDRV_teRETURN:
//         CANDRV_eRET_SUCCESS: CAN device status variable is updated and copied to
//                              ptrCANDevSts.
//
//         CANDRV_eRET_FAILURE: Error occured.

...
uint16 CAN_BUF_RX_SIZE = CANBUF_RX_PORT_BUFS;
uint16 CAN_BUF_TX_SIZE = CANBUF_TX_PORT_BUFS;
...
tucBOOL CANBUF_fnCtor( uchar8 ucPort )
{
    ...
    { 
        ...
        ptzRxFifo->uiMax = CANBUF_RX_PORT_BUFS;
        ptzRxFifo->uiCount = 0;
        ptzRxFifo->uiPut = 0;
        ptzRxFifo->uiGet = 0;
    ...
    {  
        ...
        ptzTxFifo->uiMax = CANBUF_TX_PORT_BUFS;
        ptzTxFifo->uiCount = 0;
        ptzTxFifo->uiPut = 0;
        ptzTxFifo->uiGet = 0; 
    }  
...
uint16 CANBUF_fnRxInsert( uchar8 ucPort,
                          CANBUF_tzRX_BUF *ptzRxBuf )
{
    ...
    if ( ptzFifo->uiCount < ptzFifo->uiMax )
    {
        // Copy data to frame in fifo
        (void) memcpy( &ptzFifo->ptzBuf[ ptzFifo->uiPut ],
                       ptzRxBuf,
                       sizeof( *ptzRxBuf ) );

        // Move insert point and test for wrapping
        if ( ++ptzFifo->uiPut >= ptzFifo->uiMax )
        {
            // Wrap insert point
            ptzFifo->uiPut = 0;
        }
        ...
        return( ptzFifo->uiMax - ptzFifo->uiCount );
    }
    else
    {
        // Buffer was full
...
uint16 CANBUF_fnRxExtract( uchar8 ucPort,
                          CANBUF_tzRX_BUF *ptzRxBuf )
{
    CANBUF_tzRX_FIFO *ptzFifo;
    ...
    if ( ptzFifo->uiCount > 0 )
    {
        // Copy data from fifo
        (void) memcpy( ptzRxBuf,
                       &ptzFifo->ptzBuf[ ptzFifo->uiGet ],
                       sizeof( *ptzRxBuf ) );

        // Move extraction point and test for wrapping
        if ( ++ptzFifo->uiGet >= ptzFifo->uiMax )
        {
            // Wrap extract point
            ptzFifo->uiGet = 0;
        }
...
uint16 CANBUF_fnTxInsert( uchar8 ucPort,
                          CANBUF_tzTX_BUF *ptzTxBuf )
{
...
uint16 CANBUF_fnTxExtract( uchar8 ucPort,
                          CANBUF_tzTX_BUF *ptzTxBuf )
{
    CANBUF_tzTX_FIFO *ptzFifo;
...
uchar8 CANBUF_fnTxFull( uchar8 ucPort )
{
    ...
    return ( ptzFifo->uiCount >= ptzFifo->uiMax );
}

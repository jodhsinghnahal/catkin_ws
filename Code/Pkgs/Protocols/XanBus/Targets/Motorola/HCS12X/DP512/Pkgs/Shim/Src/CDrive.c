                                        ...
                                        uchar8  ucBitRate)
{
    ...
    if (!bIsPortValid(u8PortNumber))
        return TFXCR_INVALID_PORT;
    return DRVAPI_i16InitController(u8PortNumber,
                                    ucBitRate);
}
...
{
    return DRVAPI_fnGetCommStats( Channel, ptzStats );
}
...
{
    return DRVAPI_fnClearCommStats( Channel );
}
...
TFXCAN_RETURNS TFXDRV_i16SendFrameCheck(CANPORT u8PortNumber)
{
    ...
    if ( CANBUF_fnTxFull( u8PortNumber ) )
    {
		...
		ret = TFXDRV_WriteFrame(u8PortNumber);
    }
